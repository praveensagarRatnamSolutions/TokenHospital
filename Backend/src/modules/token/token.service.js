...
const autoAssignDoctor = async (hospitalId, departmentId, date) => {
  ...
  const countMap = Object.fromEntries(
    queueCounts.map((q) => [q._id.toString(), q.count])
  );
...
const createToken = async (tokenData) => {
  ...
  try {
    ...
    let assignedDoctorId =
      doctorId ||
      (await autoAssignDoctor(hospitalId, departmentId, targetDate));
    ...
    if (tokenData.existingPaymentId) {
      payment = await Payment.findByIdAndUpdate(
        ...
        { tokenId: token._id, patientId: patient._id },
        { session, new: true }
      );
    } else if (method) {
    ...
    const populated = await Token.findById(token._id)
      .populate('departmentId doctorId patientId hospitalId')
      .lean();
...
const getDoctorQueue = async (hospitalId, doctorId, filters = {}) => {
  ...
  return {
    ...
    message:
      tokens.length === 0
        ? 'Hi! You have no patients in the queue. Have a great day!'
        : null,
  };
};
...
const skipToken = async (tokenId, hospitalId, doctorId) => {
  ...
  const nextInQueue = await Token.findOne(
    { doctorId, hospitalId, status: 'WAITING', appointmentDate },
    null,
    { sort: { isEmergency: -1, sortKey: 1 } }
  );
  ...
  const newSortKey = nextInQueue
    ? new Date(
        new Date(nextInQueue.sortKey || nextInQueue.createdAt).getTime() + 1
      )
    : new Date();
...
const getTokens = async (hospitalId, filters = {}) => {
  ...
  const pipeline = [
    ...
    {
      $sort: filters.isQueue
        ? { isEmergency: -1, sortKey: 1 }
        : { createdAt: -1 },
    },
    {
      $facet: {
...
const getPrintTokenById = async (tokenId, hospitalId) => {
  const token = await Token.findOne({ _id: tokenId, hospitalId })
    .populate('hospitalId departmentId doctorId patientId paymentId')
    .lean();

  if (!token) return null;

  const hospital = token.hospitalId || {};
  const department = token.departmentId || {};
  const doctor = token.doctorId || {};
  const patient = token.patientId || {};
  const payment = token.paymentId || {};

  return {
    hospital: {
      name: hospital.name || null,
      logo: hospital.logo || hospital.logoUrl || null,
    },
    patient: {
      name: patient.name || null,
      phone: patient.phone?.full || patient.phone || null,
      age: patient.age || null,
      gender: patient.gender || null,
    },
    doctor: {
      name: doctor.name || null,
      specialization: doctor.specialization || doctor.education || null,
      roomFloor: doctor.roomFloor || null,
    },
    token: {
      number: token.tokenNumber || null,
      sequence: token.sequenceNumber || null,
      status: token.status ? token.status.toLowerCase() : null,
    },
    department: {
