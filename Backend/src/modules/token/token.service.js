...
const { emitToDoctor } = require('../../socket/socket.service');
const socketConstant = require('../../socket/socket.constants');
const { invalidateDoctorCache } = require('../kiosk/kiosk.service');
...
const createToken = async (tokenData) => {
  ...
  try {
    ...
    // Socket event to client
    if (status !== 'PROVISIONAL') {
      const { isEmergency, _id, status, doctorId, tokenNumber } =
        token.toObject();
      emitToDoctor({
        data: {
          doctorId,
          tokenNumber,
          isEmergency,
          status: isEmergency ? 'EMERGENCY' : status,
          _id,
        },
        doctorId,
        event: socketConstant.createKioskToken,
      });
...
const completeToken = async (tokenId, hospitalId, consultationData = {}) => {
  ...
  try {
    ...
    const { _id, status, tokenNumber, doctorId, isEmergency } =
      token.toObject();

    const docId = doctorId._id.toString();

    emitToDoctor({
      event: socketConstant.updateKioskToken,
      doctorId: docId,
      data: {
        _id,
        status,
...
const callNextToken = async (doctorId, hospitalId) => {
  ...
  try {
    ...
    broadcastKioskQueue(hospitalId);
    const { _id, status, doctorId, tokenNumber, isEmergency } =
      nextToken.toObject();
    const docId = doctorId._id.toString();
    emitToDoctor({
      doctorId: docId,
      data: { _id, status, doctorId: docId, tokenNumber, isEmergency },
      event: socketConstant.updateKioskToken,
    });
  } catch (e) {
    // Socket not initialized yet, skip
