...
const StepPaymentSelection: React.FC<StepPaymentSelectionProps> = ({
  department,
  doctor,
  onProceed,
  onBack,
  ...
  const parsePhoneNumber = (phoneStr: string): PhoneData => {
    ...
    // Assumes format like "+917862145632" or "917862145632" or "7862145632"
    const fullPhone = phoneStr.startsWith("+") ? phoneStr : "+91" + phoneStr;
    const countryCode = "+91";
    const nationalNumber =
      phoneStr.length === 10 ? phoneStr : phoneStr.replace(/^\+?91/, "");
        ...
        <div className="max-w-4xl mx-auto space-y-12 pb-20">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="relative group">
                  <User
                    size={24}
                    className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 dark:text-white/10 group-focus-within:text-sky-500 transition-colors"
                  />
                  <input
                    type="text"
                    value={name}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-8">
              ...
              >
                <Banknote
                  className={`size-16 mb-4 transition-colors ${method === "CASH" ? "text-sky-500" : "text-slate-300 dark:text-white/20"}`}
                />
                <span
                  className={`text-xl font-black uppercase tracking-widest ${method === "CASH" ? "text-slate-900 dark:text-white" : "text-slate-300 dark:text-white/20"}`}
                >
                  Cash at Counter
                </span>
                <span
                  className={`text-[10px] uppercase font-bold tracking-widest mt-2 ${method === "CASH" ? "text-sky-600/60 dark:text-white/40" : "text-slate-200 dark:text-white/10"}`}
                >
