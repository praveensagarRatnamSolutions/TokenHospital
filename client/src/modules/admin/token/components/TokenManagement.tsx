         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white dark:bg-slate-900 p-8 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] shadow-sm">
            <div>
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Administration</span>
               </div>
               ...
               <p className="text-sm font-medium text-slate-500 mt-2 max-w-lg">
                  Create walk-in tokens, manage clinic queues, and resolve pending cash payments.
               </p>
            </div>
            <div className="flex gap-3 w-full md:w-auto">
               <button onClick={() => setIsModalOpen(true)} className="w-full md:w-auto px-6 py-4 bg-primary text-white font-bold rounded-2xl hover:bg-primary/90 transition-colors shadow-xl shadow-primary/30 flex items-center justify-center gap-2 group">
                  <Plus className="w-5 h-5 group-hover:rotate-90 transition-transform" /> Generate Token
               </button>
            </div>
         </div>
               ...
               <table className="w-full text-left border-collapse">
                  <thead>
                     <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Token ID</th>
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Patient File</th>
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400">Doctor / Dept</th>
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400 text-center">Status</th>
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400 text-center">Payment</th>
                        <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-400 text-right">Actions</th>
                     </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                     {isLoading ? (
                        <tr><td colSpan={6} className="p-10 text-center text-slate-400 font-medium animate-pulse">Loading tokens...</td></tr>
                        ...
                        filteredTokens.map((token: any) => (
                           <tr key={token._id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${token.isEmergency ? 'bg-red-50/30 dark:bg-red-900/5' : ''}`}>
                              ...
                              <td className="p-6">
                                 ...
                                 <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{typeof token.departmentId === 'object' ? token.departmentId?.name : 'General'}</p>
                              </td>
                              ...
                              <td className="p-6 text-center">
                                 {token.status === 'PROVISIONAL' ? (
                                    <div className="inline-flex flex-col items-center">
                                       ...
                                       <button onClick={() => handleVerifyCash(token._id)} className="text-[10px] font-bold text-primary hover:underline uppercase tracking-wide">
                                          Verify Now
                                       </button>
                                    </div>
                                 ) : (
                              ...
                              <td className="p-6 text-right">
                                 {token.status !== 'COMPLETED' && token.status !== 'CANCELED' && (
                                    <button onClick={() => handleCancelToken(token._id)} className="text-xs font-bold text-red-500 hover:text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors">
                                       Cancel
                                    </button>
                                 )}
                              </td>
...
function StatusBadge({ status }: { status: string }) {
   ...
   return (
      <span className={`px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest inline-flex items-center justify-center w-24 ${styles[status]}`}>
         {status}
      </span>
   );
}
