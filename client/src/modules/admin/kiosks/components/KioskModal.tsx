...
import { X, Save, Monitor, MapPin, Users, User, PlayCircle, Plus, Trash2, Settings } from 'lucide-react';
...
export const KioskModal: React.FC<KioskModalProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ name: '', code: '', locationType: 'general', departmentIds: [], doctorIds: [], ads: [] });
    }
  }, [initialData, isOpen]);

  if (!isOpen) return null;

  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSubmit({ ...formData, hospitalId }); };
  ...
  const addAdToPlaylist = (adId: string) => {
    ...
    setFormData(prev => ({ ...prev, ads: [...prev.ads, { adId, order: prev.ads.length }] }));
  };
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Kiosk Name</label>
                    <input
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Hardware Code</label>
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 transition-all font-mono font-black uppercase ${
                        initialData ? 'opacity-50 cursor-not-allowed' : 'focus:ring-4 focus:ring-primary/10 outline-none'
                      }`}
                      value={formData.code}
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Deployment Zone</label>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <section className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Link Departments</h3>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {loadingDepts ? (
                      ...
                      >
                        {dept.name}
                        {formData.departmentIds.includes(dept._id!) && <Plus className="w-3.5 h-3.5 rotate-45" />}
                      </button>
                    ))}
                  </div>
                </section>
                ...
                <section className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Link Doctors</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {loadingDocs ? (
                      <div className="col-span-2 text-center py-4 animate-pulse text-slate-400">Loading doctors...</div>
                    ) : doctors.length === 0 ? (
                      <div className="col-span-2 text-center py-4 text-slate-400 dark:text-slate-500 text-xs italic">
                        {formData.departmentIds.length > 0 ? 'No doctors found in these departments.' : 'Select a department first.'}
                      </div>
                        ...
                        <div className="size-10 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-700 flex-shrink-0 border border-slate-200 dark:border-slate-600">
                          {doc.profilePic ? (
                            <img
                              ...
                              alt={doc.name} className="size-full object-cover"
                            />
                          ) : (
            ...
            {activeTab === 'playlist' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <section className="space-y-4">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Playlist Sequence</h3>
                  <div className="space-y-2 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-3xl p-4 min-h-[150px]">
                    {formData.ads.length === 0 ? (
                      ...
                      formData.ads.map((item, index) => {
                        ...
                        return (
                          <div key={item.adId} className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm group">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-black truncate text-slate-900 dark:text-white">{ad?.title || 'Unknown Media'}</p>
                              <p className="text-[10px] text-slate-400 uppercase font-bold">{ad?.type} • {ad?.displayArea}</p>
                            </div>
                            <div className="flex items-center gap-1">
                              <button type="button" onClick={() => moveAd(index, 'up')} disabled={index === 0} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg disabled:opacity-20 transition-colors">
                                <Plus className="w-3.5 h-3.5 rotate-180" />
                              </button>
                              <button type="button" onClick={() => moveAd(index, 'down')} disabled={index === formData.ads.length - 1} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg disabled:opacity-20 transition-colors">
                                <Plus className="w-3.5 h-3.5" />
                              </button>
                              <button type="button" onClick={() => removeAdFromPlaylist(item.adId)} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg ml-1 transition-colors">
                ...
                <section className="space-y-4">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Available Advertisements</h3>
                  <div className="grid grid-cols-2 gap-3 max-h-[200px] overflow-y-auto pr-2">
                    {loadingAds ? (
                      <div className="col-span-2 text-center py-4 text-slate-400 animate-pulse">Loading ads...</div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[11px] font-black truncate text-slate-900 dark:text-white">{ad.title}</p>
                          <span className="text-[9px] font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded uppercase">{ad.displayArea}</span>
                        </div>
