      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-white">SMS Bundles</h2>
          </div>
          ...
          {smsPackages.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {smsPackages.map((pkg) => (
                <PackageCard key={pkg._id} pkg={pkg} onEdit={() => router.push(`/superadmin/wallet-packages/create?id=${pkg._id}`)} />
              ))}
            </div>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-white">Email Bundles</h2>
          </div>
          
          {emailPackages.length === 0 ? (
            <div className="text-center py-10 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800">
              <p className="text-slate-400 font-bold">No Email packages configured yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
function PackageCard({ pkg, onEdit }: { pkg: any, onEdit: () => void }) {
  ...
  return (
    <div className="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:-translate-y-1 transition-all relative overflow-hidden flex flex-col justify-between">
      {/* Background Decor */}
      <div className={`absolute -right-6 -top-6 w-32 h-32 rounded-full blur-3xl opacity-20 ${isSms ? 'bg-blue-500' : 'bg-purple-500'}`} />

      <div className="relative z-10 flex justify-between items-start mb-6">
        <div>
          <Badge variant={pkg.isActive ? "default" : "secondary"} className={`rounded-lg font-black tracking-widest text-[9px] uppercase ${pkg.isActive ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' : ''}`}>
        ...
        <Button variant="ghost" size="icon" onClick={onEdit} className="rounded-xl text-slate-400 hover:text-primary hover:bg-slate-50 dark:hover:bg-slate-800">
